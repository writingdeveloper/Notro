...
- **등록:** **＋** 버튼에 디스코드 이모지 우클릭 → *"링크 복사"* URL을 붙여넣거나,
  메시지에서 복사한 `<:이름:id>` 이모지 텍스트를 그대로 넣거나,
  이미지 파일을 피커에 끌어다 놓거나, **감시 폴더**(⚙)를 추가하면 폴더 안
  PNG/GIF/WebP/APNG가 해당 탭에 자동 표시됩니다.
- **저장 위치:** 등록 창의 컬렉션 목록에서 어디에 넣을지 고르고, "새 컬렉션…"으로
  새 이름을 만들 수도 있습니다. 기본값은 보고 있던 컬렉션이며, 드래그앤드롭과
  <kbd>Ctrl</kbd>+<kbd>V</kbd> 등록도 같은 규칙을 따릅니다. 등록한 항목은 보고 있던
  탭에 들어갑니다.
- **폴더째 넣기:** `%APPDATA%\Notro\assets` 아래에 폴더를 만들어 이미지를 넣으면
  등록 절차 없이 그 폴더명이 컬렉션으로 나타납니다. 움직이는 파일은 GIF 탭,
  정지 이미지는 이모지 탭에 표시됩니다.
...
- **사용:** 클릭하면 피커가 닫히고 디스코드로 포커스가 돌아가 입력창에 이미지가
  첨부됩니다. **전송(Enter)은 직접 누릅니다**(설정에서 자동 전송을 켤 수 있습니다).
  우클릭 → 이름·키워드 수정, "링크로 붙여넣기"(CDN 등록 항목), 삭제.
- 같은 탭·컬렉션에 이미 있는 그림을 다시 넣으면 조용히 쌓이지 않고 알려줍니다.
  피커 창은 크기를 조절할 수 있고 그 크기를 기억합니다.
- **크기는 항상 일정하게:** 디스코드는 첨부 이미지를 원본 픽셀 크기 그대로 그리므로,
  붙여넣기 직전에 긴 변을 맞춥니다 — 이모지 **48px**(점보 이모지와 같은 크기),
  스티커 **160px**, GIF는 원본. 탭별로 피커 설정(⚙)에서 바꿀 수 있고 "원본"도
  고를 수 있습니다. **라이브러리 원본 파일은 그대로 두고** 클립보드에 올라가는
  사본만 조정합니다.
- 움직이는 APNG 스티커는 등록 시 GIF로 자동 변환됩니다 (디스코드는 업로드된
  APNG를 재생하지 않으므로).
- 이름·키워드 검색(**한글 초성 지원** — `ㅁㅋ` → `미쿠`), "최근 사용" 섹션 지원.
- **키보드만으로:** 방향키로 그리드 이동, <kbd>Enter</kbd>로 선택 항목 붙여넣기,
  <kbd>Esc</kbd>로 닫기 — 핫키로 연 뒤 마우스가 필요 없습니다.
...
그 창에서 **구간 자르기**(`시작 – 끝`, mm:ss)와 **오디오 제거**를 바로 지정할 수 있고,
입력하는 대로 예상 결과가 갱신됩니다. 같은 용량을 짧은 구간에 쓰면 화질이 크게 올라갑니다
— 30초짜리 1080p60 영상은 전체로는 1080p30으로 떨어지지만, 7초만 남기면 **1080p60**을
그대로 유지합니다.
...
- **복사한 내용은 어디로도 전송되지 않습니다.** 텔레메트리·분석·크래시 리포트·계정,
  어느 것도 없습니다.
- **요청하지 않으면 저장하지도 않습니다.** *새 클립보드 이미지 자동 저장*은 기본
  꺼짐이며, 꺼진 상태에서는 직접 등록한 항목만 저장됩니다.
- 외부 통신은 딱 네 곳입니다: GitHub(업데이트 확인), GitHub 릴리스 자산(인스톨러 —
  실행 전에 SHA-256 검증), `cdn.discordapp.com`(링크로 이모지를 등록할 때만),
  PyPI(비디오를 처음 압축할 때 ffmpeg 조달, 역시 SHA-256 검증).
- 디스코드 클라이언트·계정·토큰은 일절 건드리지 않습니다.
- **제거해도 라이브러리는 일부러 남깁니다** — `%APPDATA%\Notro`와 설정
  `HKCU\Software\Notro`가 유지되어 재설치해도 이모지를 잃지 않습니다. 완전히 지우려면
  직접 삭제하세요.

파일이 정확히 어디에 저장되는지 등 자세한 내용은 [SECURITY.md](SECURITY.md)에 있습니다.
...
[MIT](LICENSE). 배포물에 포함된 서드파티 구성요소와 각각의 라이선스는
— LGPL-3.0인 pystray를 포함해 — [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)에
정리해 두었습니다.
